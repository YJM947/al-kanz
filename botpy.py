...
def setup_logging():
    ...
    formatter = logging.Formatter('{"time": "%(asctime)s", "level": "%(levelname)s", "message": "%(message)s", "module": "%(module)s", "func": "%(funcName)s"}')
    file_handler = logging.handlers.RotatingFileHandler('bot.log', maxBytes=5*1024*1024, backupCount=5)
...
class TradingBot:
    async def load_state_async(self):
        """Load persisted state from DB (call after event loop starts)."""
        self.balance_usd = float(await self.db.get_state('balance_usd', self.balance_usd))
        self.trading_enabled = bool(await self.db.get_state('trading_enabled', self.trading_enabled))
        self.day_key = await self.db.get_state('day_key', self.day_key)
        self.start_equity_today = float(await self.db.get_state('start_equity_today', self.start_equity_today))
        self.daily_pnl = float(await self.db.get_state('daily_pnl', self.daily_pnl))
        self.daily_trades = int(await self.db.get_state('daily_trades', self.daily_trades))
    ...
    def __init__(self):
        ...
        # Defaults; load persisted state asynchronously after loop starts
        self.balance_usd = float(TRADING_CONFIG["starting_balance_usd"])
